        ...
        <div className="space-y-12 max-w-4xl mx-auto">
            <div className="text-center space-y-6">
                <div className="flex items-center justify-center">
                    <div className={`p-5 ${content.bg} ${content.color} rounded-2xl`}>
                        <content.icon size={36} />
                    </div>
                </div>
                <h1 className="beva-heading-xl">{content.title}</h1>
                <p className="text-sm font-semibold text-[var(--text-tertiary)] uppercase tracking-wider">{content.subtitle}</p>
            </div>
            ...
            <div className="space-y-6">
                {content.sections.map((section, i) => (
                    <div
                        key={i}
                        className="beva-card-hover p-8 flex gap-6 items-start"
                    >
                        <div className={`mt-1 shrink-0 w-10 h-10 ${content.bg} rounded-xl flex items-center justify-center ${content.color}`}>
                            <section.icon size={20} />
                        </div>
                        <div className="space-y-2">
                            <h3 className="beva-heading-sm">{section.title}</h3>
                            <p className="beva-text-body">{section.content}</p>
                        </div>
                    </div>
                ))}
            </div>
